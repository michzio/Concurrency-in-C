...
static void remove_worker(thread_pool_t *thread_pool, pthread_t worker_thread) {
    ...
    dl_list_push_front(thread_pool->deleted_workers, (void *) worker_thread, sizeof(worker_thread));
...
static void join_deleted_workers(thread_pool_t *thread_pool, int no_mutex) {
    ...
    while( (node = dl_list_back(thread_pool->deleted_workers)) != NULL )
    {
        worker_thread = (pthread_t) dl_unwrap_data(node, NULL);
        dl_list_pop_back(thread_pool->deleted_workers);
...
result_t thread_pool_init(thread_pool_t **thread_pool, const size_t size, const size_t max_size, const int worker_ms_timeout) {
    ...
    *thread_pool = malloc(sizeof(thread_pool_t));
    ...
    dl_list_init(&((*thread_pool)->deleted_workers), NULL);
...
void thread_pool_shutdown(thread_pool_t *thread_pool) {
    ...
    dl_list_free(thread_pool->deleted_workers);
